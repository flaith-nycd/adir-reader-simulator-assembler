...
;/////////////////////////////////////////////////////////////////////////////////
;***Go-Scintilla***  Version 2.0 beta 3.
;*=================
;*
...
;*
;*  Platforms:  All (though code generation and call-tips probably will not work on MacOSX).
;*  Minimum version of Purebasic required: 4.5.
;/////////////////////////////////////////////////////////////////////////////////
...
GOSCI_ProcessCodeCompletionCalltipsXXX_L4:
                      If nPos ;Any non-whitespace character acts as the open call-tip separator.
                              ;In this case we check that the specified character is not the associated close call-tip separator.
...
GOSCI_StyleNextSymbolXXX_ApplyStyle:
  If blnDoNotApplyStyle = #False
    ...
    ;Was the symbol a folding symbol?
      If *keyword
        If *keyword\flags & #GOSCI_OPENFOLDKEYWORD
  ...
  For i = startLine To endLine
    ...
    *this\blnLineCodeFoldOption = 0 ;No code folding.
    ...
    ;Sort out the fold level.
      If *this\state & #GOSCI_LEXERSTATE_ENABLECODEFOLDING
        t1 = 0
...
Procedure.i GOSCI_Create(id, x, y, Width, Height, callbackAddress=0, flags=0)
  ...
  If *this
    ...
    If result
      ...
      ;Bookmarks.
        ScintillaSendMessage(*this\id, #SCI_SETMARGINMASKN, #GOSCI_MARGINNONFOLDINGSYMBOLS, 1)  ;Only marker number 0 to be allowed.
...
Procedure GOSCI_AddDelimiter(id, delimiter$, closeDelimiter$, delimiterFlags, styleIndex=#STYLE_DEFAULT)
...
Procedure GOSCI_AddKeywords(id, keyWords$, styleIndex=#STYLE_DEFAULT, keywordflags=0, blnSortKeywords=#False)
...
;Set keywordFlags to one (or more) of the keyword flags #GOSCI_OPENFOLDKEYWORD, #GOSCI_CLOSEFOLDKEYWORD or GOSCI_ADDTOCODECOMPLETION ... etc.
Procedure GOSCI_AddKeywordsEx(id, numKeywords, *ptrKeywords.STRING, *ptrCallTips.STRING=0, openCallTipSeparators$ = "", closeCallTipSeparators$="", styleIndex=#STYLE_DEFAULT, keywordFlags=0, blnSortKeywords=#False)
...
;Note that only Ascii characters must be used within each keyword.
Procedure GOSCI_RemoveKeywords(id, keyWords$)
    ...
    *this = GetGadgetData(id)
    If *this
      ;Do we need to restyle the whole document?
        If *this\state & (#GOSCI_LEXERSTATE_ENABLESYNTAXSTYLING|#GOSCI_LEXERSTATE_ENABLECODEFOLDING) <> state & (#GOSCI_LEXERSTATE_ENABLESYNTAXSTYLING|#GOSCI_LEXERSTATE_ENABLECODEFOLDING)
          blnNeedRestyle = #True
        EndIf
      *this\state = state
      If blnNeedRestyle
