...
Protected i.i, result.i = #Tok_OTHER
  ...
  If Left(LCase(sTok),1) = "$" And TypeToken = #Typ_CONST
    If IsHexadecimal(Mid(sTok,2,Len(sTok)-1))
      Result = #Tok_VALUE
      ...
      EndIf
    Else
      AddError(#ERR_TOKEN_HEXA, CurrentLine)
    EndIf
  EndIf  
  
  If Left(LCase(sTok),2) = "#$" And TypeToken = #Typ_CONST
    If IsHexadecimal(Mid(sTok,3,Len(sTok)-1))
      Result = #Tok_VALUE
      If Len(sTok)-2 >= 3 And Len(sTok)-2 < 5
        ByteSize = #_WORD
      ElseIf Len(sTok)-2 >= 1 And Len(sTok)-2 < 3
        ByteSize = #_BYTE
      Else
        ByteSize = #_NONE
        Result = #Tok_ERROR
  ...
  If Left(LCase(sTok),1) = "%" And TypeToken = #Typ_CONST
    If IsBinary(Mid(sTok,2,Len(sTok)-1))
      ByteSize = #_BIN
      Result = #Tok_VALUE
    Else
      ByteSize = #_NONE
      Result = #Tok_ERROR
      AddError(#ERR_TOKEN_BINARY, CurrentLine)
    EndIf
    If Len(Mid(sTok,2,Len(sTok)-1)) > 8
      ;Faire une verif s'il n'y a pas des ',' dans la valeur
      ; exemple dfb %1100,%1,%11110111,...
      AddError(#ERR_BIN_TOOLONG, CurrentLine)
    EndIf
  EndIf
