...
Protected pstok.i = 0, ptype.i = 0, oldpstok.i = 0
Protected IsConst.i = #False  
Protected tmpTok1.s="", tmpTok2.s="", tmpTok3.s="", value.s = ""

  FirstElement(Token())
  If pstok = 1 And ptype = #Typ_CONST
    IsConst = #True
  EndIf
  pstok = Token()\TokPos
  ptype = Token()\TokType
  value = Token()\TokValue    

  While NextElement(Token())
    
    If pstok <= oldpstok ; saut de ligne
      AddElement(CleanSRC())
        CleanSRC()\IsConst = IsConst
        CleanSRC()\token1  = tmpTok1
        CleanSRC()\token2  = tmpTok2
        CleanSRC()\token3  = tmpTok3
        DEBUG_LOG("clean: "+Str(CleanSRC()\IsConst)+" | "+CleanSRC()\token1+" "+CleanSRC()\token2+" "+CleanSRC()\token3)
        DEBUG_LOG("----------")
        tmpTok1="":tmpTok2="":tmpTok3=""
      IsConst = #False
    EndIf
    ...
    Select pstok
      Case 1
        tmpTok1=value
        DEBUG_LOG("pstok: "+Str(pstok)+" | "+tmpTok1)
      Case 2
        tmpTok2=value
        DEBUG_LOG("pstok: "+Str(pstok)+" | "+tmpTok2)
      Case 3
        tmpTok3=value
        DEBUG_LOG("pstok: "+Str(pstok)+" | "+tmpTok3)
    EndSelect
...
Procedure PrintSRC_CleanLIST()
  ...
  ForEach CleanSRC()
    ...
    With CleanSRC()
      DEBUG_LOG("SRC: "+Str(\IsConst)+" | "+\token1+" "+\token2+" "+\token3)
    EndWith
